...
class CRF(nn.Module):
    ...
    def _compute_score(
        ...
        # shape: (batch_size,)
        score = self.start_transitions[tags[0]]  # 第一个标签对应的评分
        score += emissions[0, torch.arange(batch_size), tags[0]] # 第一个标签的"发射评分"

        for i in range(1, seq_length):
            # Transition score to next tag, only added if next timestep is valid (mask == 1)
            # shape: (batch_size,)
            score += self.transitions[tags[i - 1], tags[i]] * mask[i]  # 标签的"转移评分"

            # Emission score for next tag, only added if next timestep is valid (mask == 1)
            # shape: (batch_size,)
            score += emissions[i, torch.arange(batch_size), tags[i]] * mask[i]  # 每个标签的发射评分，由LSTM/Bert建模？？？

        # End transition score
        # shape: (batch_size,)   
        # 找到了一个batch中，真正最后一标签的索引
        seq_ends = mask.long().sum(dim=0) - 1    # 这里mask的作用是处理变长输入？即一个batch中序列的长度不同，通过padding + mask的方式，处理成一样的长度。
        # shape: (batch_size,)
